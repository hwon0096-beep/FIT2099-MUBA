...
const ADVANCED_STRATEGIES: CatalogueStrategy[] = [
  { name: 'Bull Call Spread', outlook: 'bullish', access: 'Premium', profile: 'Defined-risk bullish strategy', explanation: 'Buy a lower-strike call and sell a higher-strike call to reduce upfront cost while capping maximum profit.' },
  { name: 'Bear Put Spread', outlook: 'bearish', access: 'Premium', profile: 'Defined-risk bearish strategy', explanation: 'Buy a higher-strike put and sell a lower-strike put to reduce premium while capping maximum profit.' },
  { name: 'Iron Condor', outlook: 'neutral', access: 'Premium', profile: 'Range-bound strategy', explanation: 'A multi-leg strategy designed for markets expected to remain within a price range.' },
...
export default function SimpleStrategyOverview() {
  ...
  return <div className="simple-overview">
    ...
    <section className="overview-catalogue-group" aria-labelledby="advanced-strategies-title">
      ...
      <div className="overview-catalogue-grid overview-catalogue-grid--advanced">{ADVANCED_STRATEGIES.map(strategy => <CatalogueCard key={strategy.name} strategy={strategy} locked={isFree} onUnlock={() => dispatch({ type: 'open', modal: 'premium' })} />)}</div>
    </section>
