  ...
  return <main className="app-shell strategy-lab">
    ...
    <div className="strategy-lab__workspace">
      <div className="strategy-lab__main">
        ...
        <OptionsChain rows={visibleRows} search={search} selectedStrike={selected.strike} onSearch={setSearch} onSelect={selectContract} />
        <OpenPaperPositions />
      </div>
...
function OptionsChain({ rows, search, selectedStrike, onSearch, onSelect }: { rows: readonly (typeof optionChainRows)[number][]; search: string; selectedStrike: number; onSearch: (value: string) => void; onSelect: (strike: number, side: PaperOptionSide) => void }) {
  return <section className="strategy-card strategy-card--chain"><div className="strategy-card__head"><div><h2>Options Chain</h2><p>Browse Thetanuts options and place simulated trades.</p></div><span className="demo-label"><i />Demo data</span></div><div className="chain-filters"><label>Asset<select defaultValue="BTC"><option>BTC</option><option>ETH</option></select></label><label>Expiry<select defaultValue="Sep 11"><option>Sep 11</option></select></label><label>Type<select defaultValue="All"><option>All</option><option>Call</option><option>Put</option></select></label><label className="chain-filters__search">Search<input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search strike or order ID..." /></label></div><div className="strategy-table-wrap"><table className="options-chain-table"><thead><tr><th colSpan={4} className="call-heading">Calls (Buy)</th><th rowSpan={2}>Strike</th><th colSpan={4} className="put-heading">Puts (Buy)</th></tr><tr><th>Bid</th><th>Ask</th><th>Last</th><th aria-label="Call action" /><th>Bid</th><th>Ask</th><th>Last</th><th aria-label="Put action" /></tr></thead><tbody>{rows.map((row) => <tr key={row.strike} className={row.strike === selectedStrike ? 'selected' : ''}><td>{row.call[0].toLocaleString()}</td><td>{row.call[1].toLocaleString()}</td><td>{row.call[2].toLocaleString()}</td><td><button type="button" className="paper-buy paper-buy--call" onClick={() => onSelect(row.strike, 'call')}>Paper Buy</button></td><td className="strike">{row.strike.toLocaleString()}</td><td>{row.put[0].toLocaleString()}</td><td>{row.put[1].toLocaleString()}</td><td>{row.put[2].toLocaleString()}</td><td><button type="button" className="paper-buy paper-buy--put" onClick={() => onSelect(row.strike, 'put')}>Paper Buy</button></td></tr>)}</tbody></table></div></section>
}
