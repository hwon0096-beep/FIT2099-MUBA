...
export default function PortfolioPage() {
  ...
  const [liveOrders, setLiveOrders] = useState<ExplorerOrder[] | null>(null)

  // Live spot prices for the P&L column below, and the live orders list used to tell a saved
  // strategy (further down this page) apart from a stale one — same feed DiscoverPage/AnalyzePage
  // use, fetched independently of the wallet/fills search since it doesn't depend on either.
  useEffect(() => {
    loadExplorerData().then((data) => { setMarketData(data.marketData); setLiveOrders(data.orders ?? []) }).catch(() => setMarketData(undefined))
  }, [])
  ...
  return (
    <main className="app-shell">
      ...
      <SavedStrategiesSection orders={liveOrders} />
    </main>
  )
}

function SavedStrategiesSection({ orders }: { orders: ExplorerOrder[] | null }) {
